...
public:
	...
	void velocity_brush(float delta_time)
	{
		...
		const float effect_strength = 0.01f;
		...
		for (size_t i = 0; i < grid_dimension_sqr; i++)
		{
			...
			for (size_t j = 0; j < vectors_per_side; j++)
			{
				for (size_t k = 0; k < vectors_per_side; k++)
				{
					...
					float starting_pos_x = (float)x * cell_size + cell_offset * (2 * k + 1) + offset;
					float starting_pos_y = (float)y * cell_size + cell_offset * (2 * j + 1);

					DrawLineEx({ starting_pos_x,									starting_pos_y },
							   { starting_pos_x + x_component * length_multiplier,	starting_pos_y + y_component * length_multiplier},
