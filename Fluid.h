...
static constexpr int grid_dimension = 12;
		...
		for (size_t i = 0; i < grid_dimension_sqr; i++)
		{
			...
			for (size_t j = 0; j < vectors_per_side; j++)
			{
				for (size_t k = 0; k < vectors_per_side; k++)
				{
					...
					float x_component = ((div_left * (1.0f - x_distance)) + (div_right * x_distance)) * length_multiplier;

					float y_component = ((div_up * (1.0f - y_distance)) + (div_down * y_distance)) * length_multiplier;

					if (j < vectors_per_side / 2 && y > 0)
					{
						x_component += ((divergence_vectors_x[XYtoIndex(x, y - 1, N + 1)] * (1.0f - x_distance)) + (divergence_vectors_x[XYtoIndex(x + 1, y - 1, N + 1)] * x_distance)) * length_multiplier;
					}
					else if (y < N - 1)
					{
						x_component += ((divergence_vectors_x[XYtoIndex(x, y + 1, N + 1)] * (1.0f - x_distance)) + (divergence_vectors_x[XYtoIndex(x + 1, y + 1, N + 1)] * x_distance)) * length_multiplier;
					...
					if (k < vectors_per_side / 2 && x > 0)
					{
						y_component += ((divergence_vectors_y[XYtoIndex(x - 1, y, N)] * (1.0f - x_distance)) + (divergence_vectors_y[XYtoIndex(x - 1, y + 1, N)] * x_distance)) * length_multiplier;
					}
					else if (x < N - 1)
					{
						y_component += ((divergence_vectors_y[XYtoIndex(x + 1, y, N)] * (1.0f - x_distance)) + (divergence_vectors_y[XYtoIndex(x + 1, y + 1, N)] * x_distance)) * length_multiplier;
					...
					DrawLineEx({ index_x * cell_size + cell_offset * (2 * k + 1) + offset,               index_y * cell_size + cell_offset * (2 * j + 1) },
							   { index_x * cell_size + cell_offset * (2 * k + 1) + offset + x_component, index_y * cell_size + cell_offset * (2 * j + 1) + y_component},
